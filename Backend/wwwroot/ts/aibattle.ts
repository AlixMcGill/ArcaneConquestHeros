...
import hostadd from './Modules/hostaddress';
import DeckCards from '../js/Modules/deckCards';
import Gameboard from './Modules/gameboardUi';
import generateCards from './Modules/cardGenerator';
import gameboard from './Modules/gameboardUi';
import gameLogic from './Modules/gameLogic';
import diceRollling from './Modules/diceRolling';
...
function hideSelectionOptions(bool: boolean) { 
...
function deckSelect(parentElement: any) { // render deck list
...
function cycleDeckSelection(element: any) { // highlight selected deck in list
...
function difficultySelect(parentElement: any) { // render different difficulty options
    const wrapper = document.createElement('div');
    wrapper.id = 'ai-battle-difficulty-select-wrapper';
    wrapper.className = 'vertical-flex-container';


    const easyDifficulty = document.createElement('button');
    easyDifficulty.id = 'ai-battle-difficulty-select-easy';
    easyDifficulty.className = 'ai-battle-difficulty-selection'
    ...
    normalDifficulty.className = 'ai-battle-difficulty-selection'
    ...
    hardDifficulty.className = 'ai-battle-difficulty-selection'
...
function cycleDifficultySelection(elementToAddClass: any) { // highlight different difficulty options 
...
function stagePlayerData(selectedDeck: any) {
...
function findDeckIndexById(userData: any, id: any) { // finds the index of deck array using the id of deck object inside array
    const index = userData.deckData.findIndex((deck: any) => deck.id === id);
...
function findHeroIndexById(userData: any, id: any) { // finds the index of a hero using the hero.id
    const index = userData.heroData.findIndex((hero: any) => hero.id === id);
...
function convertDeckObjectToIdArray(deck: any) {
...
function cycleTurnPhase() { // this has effectively become the "Game loop"
    ...
    diceRoller.rollD20(board, "TEST DICE ROLL", (finalRoll: any) => { 
...
function checkIfDraggableByState(turnState: boolean, gamePhase: any) {
